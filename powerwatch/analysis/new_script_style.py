...
pw_df = spark.read.jdbc("jdbc:postgresql://timescale.ghana.powerwatch.io/powerwatch", query,
        properties={"user": "powerwatch", "password": "BstvflTdfbhZ", "driver":"org.postgresql.Driver"})
...
window_size = 150
#partitioning by large time slices should technically be faster for large datasets and only introduce small errors (i.e. an outage happens on a day boundary and is split)
w = Window.partitionBy(window("time","1 day")).orderBy(asc("outage_time")).rowsBetween(-1*window_size,window_size)
...
pw_df.repartition(1).write.format("com.databricks.spark.csv").mode('overwrite').option("header", "true").save("gs://dataproc-62e9df0e-fc55-4f19-95c7-72b1b597cd63-us/results/daily_outage_per_device_time_corrected2")
