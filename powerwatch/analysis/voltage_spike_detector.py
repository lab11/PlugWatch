...
query = "(SELECT core_id, time, grid_voltage FROM powerwatch WHERE time > '2019-03-01' AND (product_id = 7008 OR product_id = 7009 or product_id = 7010 or product_id = 7011 or product_id = 8462) AND grid_voltage > 10) alias"
...
pw_df = pw_df.groupBy("core_id", window("time", '30 seconds')).avg().orderBy(desc("avg(rms_voltage)"))
pw_df = pw_df.na.drop()
pw_df.show(300)
