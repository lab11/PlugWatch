...
pw_df = pw_df.select("time","outage_duration","outage_number")
pw_df = pw_df.groupBy(month("time"),dayofmonth("time")).sum().orderBy(month("time"),dayofmonth("time"))
pw_df.repartition(1).write.format("com.databricks.spark.csv").option("header", "true").save("daily_outages_aggregate")
