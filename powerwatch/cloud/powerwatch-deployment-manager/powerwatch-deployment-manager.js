...
function fetchSurveys(formid, date, callback) {
   ...
   var uri = ""
   if(date == 0) {
      uri = 'https://gridwatch.surveycto.com/api/v1/forms/data/wide/json/' + formid
   } else {
      uri = 'https://gridwatch.surveycto.com/api/v1/forms/data/wide/json/' + formid + '?date=' + String(date)
   }
   ...
   request(options, function(error, response, body) {
      json = JSON.parse(body,null,2)
      
      for(var i = 0; i < json.length; i++) {
         //parse out the picture if it exists
         if(json[i].g_deviceQR != '') {
            num_images += 1
            //console.log(json[i].g_deviceQR)
            var options = {
               uri: json[i].g_deviceQR,
               auth: {
                  user: survey_config.username,
                  pass: survey_config.password,
                  sendImmediately: false
...
function fetchNewSurveys() {
   ...
   fetchSurveys('Combined_Form', lastSurveyFetch, function(data) {
      //processPowerwatchEntrySurvey(data)
   })
